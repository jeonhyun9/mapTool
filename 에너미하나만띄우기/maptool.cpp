...
HRESULT maptool::init()
{
	...
	enemyImg = IMAGEMANAGER->findImage("회색콩벌레_수정");
...
void maptool::render()
{
	...
	for (int i = 0; i < TILEX * TILEY; i++)
	{
		if (IntersectRect(&rcTemp, &rcWins, &sTile[i].rc))
		{
			...
			//enemyImg->frameRender(getMemDC(), sTile[i].rc.left, sTile[i].rc.top,
			//	sTile[i].enemyFrameX, sTile[i].enemyFrameY);
		}
	}
	...
	for (int i = 0; i < TILEX * TILEY; i++)
	{
		...
		enemyImg->render(getMemDC(), sTile[i].rc.left, sTile[i].rc.top);
	}
