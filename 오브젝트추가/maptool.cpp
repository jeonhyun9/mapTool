...
HRESULT maptool::init()
{
	...
	IMAGEMANAGER->addImage("백그라운드", "images/배경화면3배.bmp", 6000, 960); 
	IMAGEMANAGER->addImage("미니맵", "images/배경화면3배미니.bmp", 600, 96); 
		...
		if (INPUT->GetKeyDown(VK_LBUTTON))
		{
			...
			if (PtInRect(&sToolBtn.rcDelete, _ptMouse))
			{
				...
				else
				{
					...
					if (isTerrain)			//현구짱 이레이저 예외처리 드래그
					{
						ctrlSelect = CTRL_TERRAIN;
					}
					else if (isObject)
			...
			{
				isTerrain = true;
				isObject = false; //오브젝트 상태 변환 -동주 
				ctrlSelect = CTRL_TERRAIN;
			}
			if (PtInRect(&sToolBtn.rcObject, _ptMouse))
			{
				isObject = true;
				isTerrain = false; //터레인 상태 변환 -동주 
				ctrlSelect = CTRL_OBJECT;
			}
